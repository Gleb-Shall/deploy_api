...
server {
    ...
    location / {
        ...
        sub_filter_once on;
        sub_filter '</head>' '<script src="https://automatoria.ru/api/app-state.js?site_id=${SITE_PATH}&v=3" defer></script></head>';
...
server {
    ...
    location / {
        ...
        sub_filter_types text/html;
        sub_filter_once on;
        sub_filter '</head>' '<script src="https://automatoria.ru/api/app-state.js?site_id=${SITE_PATH}&v=3" defer></script><script src="https://automatoria.ru/api/preview-js?h=${SITE_PATH}"></script></head>';
        proxy_pass http://127.0.0.1:${PORT}/;
        proxy_http_version 1.1;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
